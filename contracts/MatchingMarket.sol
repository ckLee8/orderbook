...
contract MatchingMarket is MatchingEvents, SimpleMarket {
        ...
        returns (uint fill_amt)
    {
        ...
        while (pay_amt > 0) {                           //while there is amount to sell
            ...
            if (pay_amt * 1 ether < (offers[offerId].buy_amt / offers[offerId].pay_amt)) {
            ...
            if (pay_amt >= offers[offerId].buy_amt) {                       //If amount to sell is higher or equal than current offer amount to buy
                ...
                uint256 baux = ((pay_amt * 10 ** 9) * (offers[offerId].pay_amt / offers[offerId].buy_amt)) / 10 ** 9;
        ...
        returns (uint fill_amt)
    {
        ...
        while (buy_amt > 0) {                           //Meanwhile there is amount to buy
            ...
            if (buy_amt * 1 ether < (offers[offerId].pay_amt / offers[offerId].buy_amt)) {
                break;                                  //We consider that all amount is sold
            }
            if (buy_amt >= offers[offerId].pay_amt) {                       //If amount to buy is higher or equal than current offer amount to sell
                ...
                fill_amt = (fill_amt + ((buy_amt * 10 ** 9) * (offers[offerId].buy_amt / offers[offerId].pay_amt)) / 10 ** 9); //Add amount sold to acumulator
    ...
    function getBuyAmount(ERC20 buy_gem, ERC20 pay_gem, uint pay_amt) public view returns (uint fill_amt) {
        ...
        fill_amt = (fill_amt + ((pay_amt * 10 ** 9) * (offers[offerId].pay_amt / offers[offerId].buy_amt)) / 10 ** 9); //Add proportional amount of last offer to buy accumulator
    }
    ...
    function getPayAmount(ERC20 pay_gem, ERC20 buy_gem, uint buy_amt) public view returns (uint fill_amt) {
        ...
        fill_amt = (fill_amt + ((buy_amt * 10 ** 9) * (offers[offerId].buy_amt / offers[offerId].pay_amt)) / 10 ** 9); //Add proportional amount of last offer to pay accumulator
    }
